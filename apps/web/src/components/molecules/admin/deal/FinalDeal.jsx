...
const FinalDeal = ({ deal }) => {
  ...
  return (
    <div className='relative h-full'>
      <div  onClick={() => {router.push(`/seller/deals/${deal.id}`)}} className={`absolute cursor-pointer left-8 z-10 -top-3 border  rounded px-10 text-sm  border-purple-500 bg-purple-200 text-purple-700`}>
        ICT{deal.id}
      </div>
      <div className={`border h-full border-purple-500 bg-white rounded p-6 flex flex-col gap-5`}>
        ...
        <section className='flex justify-between gap-10 text-gray-700 cursor-pointer font-medium mt-4'>
          <div className='flex justify-between gap-6'>
            <Button color='secondary' onClick={() => { setShowMore("SUMMARY") }} >
              <div>Summary</div>
            </Button>
            <Button color='secondary' onClick={() => { setShowMore("KNOW MORE") }} >
              <div>Know More</div>
            </Button>
          </div>
        </section>
